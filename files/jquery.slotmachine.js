	...
	*/
	SlotMachine.prototype.shuffle = function( spins, onComplete ){		
		...
		*/
		...
		if (showed != null) {
			for (var i = 0; i < 9; i++) {
				for(var key in showed){
					//console.log(key);
        			if (showed[key] == this.futureActive) {
        				//console.log(key); 
        				var rnd = this.getCustom();
						this.futureActive = rnd;
						for (var k in showed) {
							if (showed[k] == this.futureActive) {
        						ok = false;
							}
						}
						if (ok) {
		...
		showed.push(this.futureActive);
		if (showed.length > this.$tiles.length) {
			//console.log("showed.length = " + showed.length);
